...
* > Created Time: Sat 20 Jun 2015 10:04:04 PM PDT
*/
...
namespace Socket
{
    ...
    void TCP::listen_on_port(Port port, unsigned int listeners = 1)
    {
        ...
        if (listen(this->_socket_id, listeners) != 0)
        {
            ...
            << "] [listeners=" << listeners << "] Cannot bind socket";
    ...
    void TCP::connect_to(Address address)
    {
        ...
        if(connect(this->_socket_id, (struct sockaddr *)&address,
                   sizeof(struct sockaddr_in)) < 0)
        {
            stringstream error;
            error << "[connect_to] with [address=" << address
    ...
    TCP TCP::accept_client()
    {
        ...
        ret.close();
        ret._socket_id = accept(this->_socket_id,
                                (struct sockaddr *)&(ret._address), &len);
        ret._opened = true;
        ret._binded = true;
    ...
    {
        if(!this->_binded)
            throw SocketException("[Send] socket not bind");
        if(!this->_opened)
            throw SocketException("[Send] socket not open");

        len *= sizeof(T);
        if(len > (SOCKET_MAX_BUFFER_LEN *sizeof(T)))
        {
            stringstream error;
            error << "[send] length higher than max len";
            throw SocketException(error.str());
        }

        int ret;
        if((ret = ::send(this->_socket_id, (const char *)buffer, len, 0)) == -1)
            throw("[send] Cannot send");
        return ret;
    }
    ...
    {
        if(!this->_binded)
            throw SocketException("[receive] socket not bind");
        if(!this->_opened)
            throw SocketException("[receive] socket not open");
            
        len *= sizeof(T);
        if(len > (SOCKET_MAX_BUFFER_LEN *sizeof(T)))
        {
            stringstream error;
            error << "[receive] length higher than max len";
            throw SocketException(error.str());
        }
